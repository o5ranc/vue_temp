...
import { DiffModeEnum, DiffView, SplitSide, DiffFile } from '@git-diff-view/vue'
...
const extendData = ref<DiffViewProps<any>['extendData']>({
  oldFile: {},
  newFile: {},
})
...
<template>
  ...
  <div
    class="m-auto mb-[5em] w-[90%] overflow-hidden rounded-[5px] border border-solid border-[#c1c1c1]"
  >
    <DiffView
      :diff-file="diffFile"
              ...
              <!-- <button
                ...
                @click="
                  () => {
                    if (v.length) {
                      const _side =
                        side === SplitSide.old ? 'oldFile' : 'newFile'
